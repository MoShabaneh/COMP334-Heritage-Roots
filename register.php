...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["name"])) {
        ...
        $stmt = $pdo->prepare("INSERT INTO customer (name, h_num, street, city, country, dob, id, email, tel, card_num, exp_date, card_name, bank) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$_SESSION["name"], $_SESSION["h_num"], $_SESSION["street"], $_SESSION["city"], $_SESSION["country"], $_SESSION["dob"], $_SESSION["id_number"], $_SESSION["email"], $_SESSION["telephone"], $_SESSION["cc_number"], $_SESSION["cc_expiry"], $_SESSION["cc_name"], $_SESSION["cc_bank"]]);

        // Insert into account table
        $stmt = $pdo->prepare("INSERT INTO account (username, pass, gov_id) VALUES (?, ?, ?)");
        $stmt->execute([$_SESSION["username"], $_SESSION["password"], $_SESSION["id_number"]]);
...
<body>
    ...
    <?php elseif ($phase == 3) : ?>
        ...
        <?php else: ?>
            ...
            <p>Address: <?php echo $_SESSION["h_num"] . " " . $_SESSION["street"] . ", " . $_SESSION["city"] . ", " . $_SESSION["country"]; ?></p>
